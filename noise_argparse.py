...
def get_noise_layers(hmin,hmax,wmin,wmax):
    layers = []
    layers.append(Crop((hmin, hmax), (wmin, wmax)))

    # layers.append(Cropout((hmin, hmax), (wmin, wmax)))
    # layers.append(Dropout((0.8, 1)))
    layers.append(Resize((0.5, 0.9)))
    layers.append('JpegPlaceholder')
    layers.append(Blur(2))
    layers.append('QuantizationPlaceholder')
    return layers
